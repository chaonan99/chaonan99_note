...
* Incrementally optimised ranking function
	$$
	err(f_{x^p}(x^g), y)=\mathcal{L}_y(rank(f_{x^p}(x^g)))
	$$
	where $f_{x^p}(x^g)$ is the distanse of pair $\{x^p, x_g\}$, which is defined as negative [Mahalanobis Distance](https://en.wikipedia.org/wiki/Mahalanobis_distance). $y$ denotes the feedback, that is, $y\in $ {true-match, strong-negative, week-negative} ({m,s,w}). $rank$ is just an int number denotes the rank of a gallery image.
* Re-id ranking loss $\mathcal{L}_y$ is defined as
	$$
	\begin{align}
	\mathcal{L}_y(k)&=\sum_{i=1}^k \alpha_i	\quad if\quad y\in \{m,w\}\\
	or &=\sum_{i=k+1}^{n_g}\alpha_i \quad if \quad y\in \{s\}
	\end{align}
	$$
	with $\alpha_1\geq \alpha_2\geq \dots \geq 0$

### Real-time Model Update for Instant Feedback Reward
* Negative Mahalanobis Distance:
	$$
	f_{x^p}(x^g)=-[(x^p-x^g)^\mathbf{T}M(x^p-x^g)], M\in S^d_+
	$$
	$S^d_+$ represents semi-definate matrix.
* Knowledge cumulation by online learning
	$$
	M_t=\underset{M\in S^d_+}{\arg\min}\Delta_F(M,M_{t-1})+\eta\mathcal{L}^{(t)}
	$$
	This equation with $t$ indicate that the matrix $M$ in M-distance is learned in stages (knowledge cumulation). $\mathcal{L}^{(t)}$ is the loss of human feed back in $t$ stage. $\Delta_F$ is [Burg matrix divergence](https://en.wikipedia.org/wiki/Bregman_divergence)
...
>>>>>>> 542c059f24f12b553d73455997bf8349fa0c06ae
